...
stop_words = set([
    ...
    'what', 'which', 'who', 'whom', 'whose', 'how', 'when', 'where', 'why','iin',
])
...
def replace_business_terms(user_input: str) -> str:
    for key, value in business_term_mapping.items():
        ...
        temp = "for every query exclude this; WHERE ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others') || Our additional recommendation for generating the SQL query:  \n"  
...
- Output queries that are complete, correct, and production-ready.
    ...
    > IMPORTANT: For every SQL query that aggregates SalesQuantity over a date range, you MUST divide the SUM(SalesQuantity) by the exact number of days in the date range using DATEDIFF(DAY, start_date, end_date). Do NOT omit this division under any circumstances... refer date_logic_and_fiscal_rules
    ...
    MAIN LOGIC:
       usecase 1: sale quantity or volume or sale : always divide 
            ...
            *3) user query : what is the sale of milk from hso 1 yesterday ? -> sql query: SELECT SUM(f.SalesQuantity)/1 AS MilkSalesQuantity_Yesterday_HSO1 FROM Dw.fsales f INNER JOIN Dw.dSalesOfficeMaster d ON f.SalesOfficeID = d.PLANT AND d.[Sales Offcie Type] = 'sales Office' WHERE CAST(f.BillingDate AS DATE) = CAST(GETDATE() - 1 AS DATE) AND f.ProductHeirachy1 = 'MILK'  -- or correct milk identifier AND d.Short_Name = 'HSO 1';
            ...
            13) eg: what is the sale of milk 500 Ml in HSO 1 yesterday? SQL : SELECT SUM(f.SalesQuantity) / 1 AS MilkSalesQuantity_Yesterday FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON f.SalesOfficeID = d.PLANT WHERE f.ProductHeirachy1 = 'Milk' AND f.ProductHeirachy5 = '500 ML' AND CAST(f.BillingDate AS DATE) = CAST(GETDATE() - 1 AS DATE) AND d.[Sales Offcie Type] = 'sales Office' AND d.SHORT_NAME = 'HSO 1' AND f.ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others');
            14) eg: which package size has the highest sale for milk yesterday ? SQL: SELECT TOP 1 ProductHeirachy5 FROM DW.fSales WHERE ProductHeirachy1 = 'Milk' AND BillingDate = CAST(GETDATE() - 1 AS DATE) GROUP BY ProductHeirachy5 ORDER BY SUM(SalesQuantity) DESC;
            15) eg: which package type has the highest sale fo milk yesterday ? SQL: SELECT TOP 1 ProductHeirachy4 FROM DW.fSales WHERE ProductHeirachy1 = 'Milk' AND BillingDate = CAST(GETDATE() - 1 AS DATE) GROUP BY ProductHeirachy4 ORDER BY SUM(SalesQuantity) DESC;
             lowest means ASC (Ascending Order) like eg: which package size has the lowest sale for milk ? SQL:  SELECT TOP 1 ProductHeirachy5 FROM DW.fSales WHERE ProductHeirachy1 = 'Milk' AND BillingDate = CAST(GETDATE() - 1 AS DATE) GROUP BY ProductHeirachy5 ORDER BY SUM(SalesQuantity) ASC;
            16) compare sale of milk vs sale of curd yesterday: SQL: SELECT SUM(CASE WHEN f.ProductHeirachy1 = 'Milk' THEN f.SalesQuantity ELSE 0 END) / 1 AS MilkSalesQuantity_Yesterday, SUM(CASE WHEN f.ProductHeirachy1 = 'Curd' THEN f.SalesQuantity ELSE 0 END) / 1 AS curdSalesQuantity_Yesterday FROM DW.fSales f WHERE CAST(f.BillingDate AS DATE) = CAST(GETDATE() - 1 AS DATE);

    IMP model: =>If the query asks for average sale per [entity], always compute: AvgSale = (SUM(SalesQuantity) ÷ number_of_days_in_period) ÷ COUNT(DISTINCT CustomerID),
    examples: 
    1) What is the avg sale per agents lastweek? SQL:SELECT CAST(SUM(f.SalesQuantity) AS FLOAT) / 7.0 / COUNT(DISTINCT f.CustomerID) AS AvgSalePerAgentLastWeek FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON f.SalesOfficeID = d.PLANT WHERE f.BillingDate BETWEEN DATEADD(DAY, -7, DATEADD(WEEK, DATEDIFF(WEEK, 0, GETDATE()), 0)) AND DATEADD(DAY, -1, DATEADD(WEEK, DATEDIFF(WEEK, 0, GETDATE()), 0)) AND f.CustomerGroup = 'Agents' AND f.ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others')
    * Remember here the keywords are : 'Per', 'Avg','average' and also values which are belongs to CustomerGroup. Then you have to follow this model.
    2) what is the avg LPD for agents for HSO 1 yesterday? SQL: SELECT CAST(SUM(f.SalesQuantity) AS FLOAT) / COUNT(DISTINCT f.CustomerID) AS AvgSalePerAgentYesterday FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON f.SalesOfficeID = d.PLANT WHERE f.BillingDate = CAST(GETDATE() - 1 AS DATE) AND f.CustomerGroup = 'Agents' AND d.Short_Name = 'HSO 1' AND f.ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others')
        THIS is like if any one asked question relating to the these columns : CITY,STATE,REGION_NAME,Short_Name then join like - join DW.fSales with DW.dSalesOfficeMaster like : select * from dw.fSales f inner join DW.dSalesOfficeMaster d on f.SalesOfficeID = d.PLANT and d.[Sales Offcie Type]='sales Office'
    3) what is the avg lpd Per  agents for lastweek in hso 1 ? SQL: SELECT CAST(SUM(f.SalesQuantity) AS FLOAT) / 7.0 / COUNT(DISTINCT f.CustomerID) AS AvgLPD FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON f.SalesOfficeID = d.PLANT WHERE f.BillingDate BETWEEN DATEADD(DAY, -7, DATEADD(WEEK, DATEDIFF(WEEK, 0, GETDATE()), 0)) AND DATEADD(DAY, -1, DATEADD(WEEK, DATEDIFF(WEEK, 0, GETDATE()), 0)) AND f.CustomerGroup = 'Agents' AND d.Short_Name = 'HSO 1' AND f.ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others')
    4) what is the avg sale per HDC in april 2025? SQL;SELECT CAST(SUM(f.SalesQuantity) AS FLOAT) / 30.0 / COUNT(DISTINCT f.CustomerID) AS AvgSalePerHDC FROM DW.fSales f WHERE f.BillingDate BETWEEN '2025-04-01' AND '2025-04-30' AND f.CustomerGroup = 'HDC' AND f.ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others')
    5) hey what is the average sale per hdc lastmonth ?: SELECT CAST(SUM(f.SalesQuantity) AS FLOAT) / DATEDIFF(DAY, DATEADD(DAY, -7, DATEADD(WEEK, DATEDIFF(WEEK, 0, GETDATE()), 0)), DATEADD(DAY, -1, DATEADD(WEEK, DATEDIFF(WEEK, 0, GETDATE()), 0))) / COUNT(DISTINCT f.CustomerID) AS AvgSalePerHDCLastWeek FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON f.SalesOfficeID = d.PLANT WHERE f.BillingDate BETWEEN DATEADD(DAY, -7, DATEADD(WEEK, DATEDIFF(WEEK, 0, GETDATE()), 0)) AND DATEADD(DAY, -1, DATEADD(WEEK, DATEDIFF(WEEK, 0, GETDATE()), 0)) AND f.CustomerGroup = 'HDC' AND f.ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others');
...
- Output queries that are complete, correct, and production-ready.
 ...
 > tables_and_schema: > Use the following tables and their core columns:
    ...
    > INSTRUCTIONS:
        ...
        - loss  making means low achievement so use  ASC filter
        - profit making means high achievement so user DESC filter
        - understand the user question properly, use date_logic_and_fiscal_rules for calculating dates etc.
        very important: - if any one asked question relating to the these columns : CITY,STATE,REGION_NAME,Short_Name then join like - join DW.fSales with DW.dSalesOfficeMaster like : select * from dw.fSales f inner join DW.dSalesOfficeMaster d on f.SalesOfficeID = d.PLANT and d.[Sales Offcie Type]='sales Office'
    ...
    **Important Note: The following examples cover common ways users may ask about Budget, target, actual etc. If a user query differs in wording or format, interpret the intent and generate the SQL by adapting the logic shown in these examples. Always use them as reference patterns for solving similar queries.**
    ...
    MODEL 1)If user **Mentioned product** : you have to definitely with the material codes for the product and do calculate
    Examples: 1) what is the budget for milk in  hso 1 in april 2025 ? -> sql : WITH MilkMaterialCodes AS (SELECT DISTINCT MaterialCode FROM DW.fSales WHERE ProductHeirachy1 = 'Milk' AND ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others')), Actual AS (SELECT SUM(f.SalesQuantity) / 30.0 AS actual FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = f.SalesOfficeID INNER JOIN MilkMaterialCodes m ON f.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND f.BillingDate BETWEEN '2025-04-01' AND '2025-04-30' AND d.short_name = 'hso 1'), Target AS (SELECT SUM(b.BudgetQuantityLPD) AS target FROM DW.fSalesBudget b INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = b.SalesOfficeID INNER JOIN MilkMaterialCodes m ON b.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND b.PlanMonth = '202504' AND d.Short_Name = 'hso 1') SELECT a.actual, t.target, CASE WHEN t.target = 0 THEN NULL ELSE (a.actual / t.target) * 100 END AS AchievementPercent FROM Actual a, Target t;   
            2) what is the budget of milk in vijayawada in april 2025 ? sql : WITH MilkMaterialCodes AS (SELECT DISTINCT MaterialCode FROM DW.fSales WHERE ProductHeirachy1 = 'Milk' AND ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others')), Actual AS (SELECT SUM(f.SalesQuantity) / 30.0 AS actual FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = f.SalesOfficeID INNER JOIN MilkMaterialCodes m ON f.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND f.BillingDate BETWEEN '2025-04-01' AND '2025-04-30' AND d.CITY = 'VIJAYAWADA'), Target AS (SELECT SUM(b.BudgetQuantityLPD) AS target FROM DW.fSalesBudget b INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = b.SalesOfficeID INNER JOIN MilkMaterialCodes m ON b.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND b.PlanMonth = '202504' AND d.CITY = 'VIJAYAWADA') SELECT a.actual, t.target, CASE WHEN t.target = 0 THEN NULL ELSE (a.actual / t.target) * 100 END AS AchievementPercent FROM Actual a, Target t;
            3) what is the budget for milk in april 2025? -> SQL : WITH MilkMaterialCodes AS (SELECT DISTINCT MaterialCode FROM DW.fSales WHERE ProductHeirachy1 = 'Milk' AND ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others')), Actual AS (SELECT SUM(f.SalesQuantity) / 30.0 AS actual FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = f.SalesOfficeID INNER JOIN MilkMaterialCodes m ON f.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND f.BillingDate BETWEEN '2025-04-01' AND '2025-04-30'), Target AS (SELECT SUM(b.BudgetQuantityLPD) AS target FROM DW.fSalesBudget b INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = b.SalesOfficeID INNER JOIN MilkMaterialCodes m ON b.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND b.PlanMonth = '202504') SELECT a.actual, t.target, CASE WHEN t.target = 0 THEN NULL ELSE (a.actual / t.target) * 100 END AS AchievementPercent FROM Actual a, Target t;
            4) what is the budget of milk in april 2025 for vijayawada? -> SQL: WITH MilkMaterialCodes AS (SELECT DISTINCT MaterialCode FROM DW.fSales WHERE ProductHeirachy1 = 'Milk' AND ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others')), Actual AS (SELECT SUM(f.SalesQuantity) / 30.0 AS actual FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = f.SalesOfficeID INNER JOIN MilkMaterialCodes m ON f.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND f.BillingDate BETWEEN '2025-04-01' AND '2025-04-30' AND d.STATE = 'ANDHRA PRADESH'), Target AS (SELECT SUM(b.BudgetQuantityLPD) AS target FROM DW.fSalesBudget b INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = b.SalesOfficeID INNER JOIN MilkMaterialCodes m ON b.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND b.PlanMonth = '202504' AND d.STATE = 'ANDHRA PRADESH') SELECT a.actual, t.target, CASE WHEN t.target = 0 THEN NULL ELSE (a.actual / t.target) * 100 END AS AchievementPercent FROM Actual a, Target t;
            5) what is budget of milk in UHT Poly Pack in april 2025 in hso 1?-> SQL: WITH MilkMaterialCodes AS (SELECT DISTINCT MaterialCode FROM DW.fSales WHERE ProductHeirachy1 = 'Milk' AND ProductHeirachy4 = 'UHT Poly Pack' AND ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others')), DateInfo AS (SELECT DAY(EOMONTH('2025-04-01')) AS DaysInMonth), Actual AS (SELECT SUM(f.SalesQuantity) * 1.0 / MAX(di.DaysInMonth) AS actual FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = f.SalesOfficeID INNER JOIN MilkMaterialCodes m ON f.MaterialCode = m.MaterialCode CROSS JOIN DateInfo di WHERE d.[Sales Offcie Type] = 'sales Office' AND f.BillingDate BETWEEN '2025-04-01' AND '2025-04-30' AND d.Short_Name = 'HSO 1'), Target AS (SELECT SUM(b.BudgetQuantityLPD) AS target FROM DW.fSalesBudget b INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = b.SalesOfficeID INNER JOIN MilkMaterialCodes m ON b.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND b.PlanMonth = '202504' AND d.Short_Name = 'HSO 1') SELECT a.actual, t.target, CASE WHEN t.target = 0 THEN NULL ELSE (a.actual / t.target) * 100 END AS AchievementPercent FROM Actual a, Target t;
                        THIS is like if any one asked question relating to the these columns : CITY,STATE,REGION_NAME,Short_Name then join like - join DW.fSales with DW.dSalesOfficeMaster like : select * from dw.fSales f inner join DW.dSalesOfficeMaster d on f.SalesOfficeID = d.PLANT and d.[Sales Offcie Type]='sales Office'
            6) what is budget of curd in Sachets in april 2025 in hso 1?-> SQL : WITH CurdMaterialCodes AS (SELECT DISTINCT MaterialCode FROM DW.fSales WHERE ProductHeirachy1 = 'Curd' AND ProductHeirachy4 = 'Sachets' AND ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others')), DateInfo AS (SELECT DAY(EOMONTH('2025-04-01')) AS DaysInMonth), Actual AS (SELECT SUM(f.SalesQuantity) * 1.0 / MAX(di.DaysInMonth) AS actual FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = f.SalesOfficeID INNER JOIN CurdMaterialCodes m ON f.MaterialCode = m.MaterialCode CROSS JOIN DateInfo di WHERE d.[Sales Offcie Type] = 'sales Office' AND f.BillingDate BETWEEN '2025-04-01' AND '2025-04-30' AND d.Short_Name = 'HSO 1'), Target AS (SELECT SUM(b.BudgetQuantityLPD) AS target FROM DW.fSalesBudget b INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = b.SalesOfficeID INNER JOIN CurdMaterialCodes m ON b.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND b.PlanMonth = '202504' AND d.Short_Name = 'HSO 1') SELECT a.actual, t.target, CASE WHEN t.target = 0 THEN NULL ELSE (a.actual / t.target) * 100 END AS AchievementPercent FROM Actual a, Target t;
            7) what is the budget of cheese 100 gms in april 2025 in hso 5?-> SQL: WITH CheeseMaterialCodes AS (SELECT DISTINCT MaterialCode FROM DW.fSales WHERE ProductHeirachy1 = 'Cheese' AND ProductHeirachy5 = '100 GMS' AND ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others')), DateInfo AS (SELECT DAY(EOMONTH('2025-04-01')) AS DaysInMonth), Actual AS (SELECT SUM(f.SalesQuantity) * 1.0 / MAX(di.DaysInMonth) AS actual FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = f.SalesOfficeID INNER JOIN CheeseMaterialCodes m ON f.MaterialCode = m.MaterialCode CROSS JOIN DateInfo di WHERE d.[Sales Offcie Type] = 'sales Office' AND f.BillingDate BETWEEN '2025-04-01' AND '2025-04-30' AND d.Short_Name = 'HSO 5'), Target AS (SELECT SUM(b.BudgetQuantityLPD) AS target FROM DW.fSalesBudget b INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = b.SalesOfficeID INNER JOIN CheeseMaterialCodes m ON b.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND b.PlanMonth = '202504' AND d.Short_Name = 'HSO 5') SELECT a.actual, t.target, CASE WHEN t.target = 0 THEN NULL ELSE (a.actual / t.target) * 100 END AS AchievementPercent FROM Actual a, Target t;
                    -- like this you have to use ProductHeirachy5 and satisfy user requiremet`
            8) what is the budget of milk in may 2025 vs april 2025 ? SQL: WITH MilkMaterialCodes AS (SELECT DISTINCT MaterialCode FROM DW.fSales WHERE ProductHeirachy1 = 'Milk' AND ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others')), AprilActual AS (SELECT SUM(f.SalesQuantity)/30.0 AS actual FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = f.SalesOfficeID INNER JOIN MilkMaterialCodes m ON f.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND f.BillingDate BETWEEN '2025-04-01' AND '2025-04-30'), AprilTarget AS (SELECT SUM(b.BudgetQuantityLPD) AS target FROM DW.fSalesBudget b INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = b.SalesOfficeID INNER JOIN MilkMaterialCodes m ON b.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND b.PlanMonth = '202504'), MayActual AS (SELECT SUM(f.SalesQuantity)/31.0 AS actual FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = f.SalesOfficeID INNER JOIN MilkMaterialCodes m ON f.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND f.BillingDate BETWEEN '2025-05-01' AND '2025-05-31'), MayTarget AS (SELECT SUM(b.BudgetQuantityLPD) AS target FROM DW.fSalesBudget b INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = b.SalesOfficeID INNER JOIN MilkMaterialCodes m ON b.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND b.PlanMonth = '202505') SELECT a.actual AS AprilActual, t.target AS AprilTarget, CASE WHEN t.target = 0 THEN NULL ELSE (a.actual / t.target) * 100 END AS AprilAchievementPercent, ma.actual AS MayActual, mt.target AS MayTarget, CASE WHEN mt.target = 0 THEN NULL ELSE (ma.actual / mt.target) * 100 END AS MayAchievementPercent, CASE WHEN t.target = 0 OR mt.target = 0 THEN NULL ELSE ((ma.actual / mt.target) - (a.actual / t.target)) * 100 END AS AchievementGrowthPercent FROM AprilActual a, AprilTarget t, MayActual ma, MayTarget mt;
            9) what is the budget of milk of  500 ml in hso 1 in may 2025 ? SQL: SELECT SUM(f.SalesQuantity)/31.0 AS Actual, SUM(b.BudgetQuantityLPD) AS Target, CASE WHEN SUM(b.BudgetQuantityLPD) = 0 THEN NULL ELSE (SUM(f.SalesQuantity)/31.0)/SUM(b.BudgetQuantityLPD)*100 END AS AchievementPercent FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = f.SalesOfficeID INNER JOIN DW.fSalesBudget b ON b.SalesOfficeID = f.SalesOfficeID AND b.PlanMonth = '202505' INNER JOIN DW.dSalesOfficeMaster d2 ON d2.PLANT = b.SalesOfficeID INNER JOIN (SELECT DISTINCT MaterialCode FROM DW.fSales WHERE ProductHeirachy1 = 'Milk' AND ProductHeirachy5 = '500 Ml' AND ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others')) m ON f.MaterialCode = m.MaterialCode AND b.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND d2.[Sales Offcie Type] = 'sales Office' AND f.BillingDate BETWEEN '2025-05-01' AND '2025-05-31' AND d.Short_Name = 'HSO 1' AND d2.Short_Name = 'HSO 1'

    MODEL 2)If user **did not mentioned product** : you need not to consider any product(material code)
    Examples: 1) what is the budget for hyderabad in april 2025 ? (without product filter) -> sql -> WITH PlantID AS (SELECT PLANT FROM Dw.dSalesOfficeMaster WHERE CITY = 'Hyderabad' AND [Sales Offcie Type] = 'Sales Office'), TargetVolume AS (SELECT SUM(BudgetQuantityLPD) AS TargetVolume FROM Dw.fSalesBudget b INNER JOIN PlantID p ON b.SalesOfficeID = p.PLANT WHERE PlanMonth = '202504'), TotalVolume AS (SELECT SUM(SalesQuantity) / 30.0 AS TotalVolume FROM Dw.fSales f INNER JOIN PlantID p ON f.SalesOfficeID = p.PLANT WHERE BillingDate BETWEEN '2025-04-01' AND '2025-04-30') SELECT t.TargetVolume, s.TotalVolume, CASE WHEN t.TargetVolume = 0 THEN NULL ELSE (s.TotalVolume / t.TargetVolume) * 100 END AS AchievementPercent FROM TargetVolume t, TotalVolume s;
            2) what is the budget for hso 1 in april 2025 ? -> sql -> WITH PlantID AS (SELECT PLANT FROM Dw.dSalesOfficeMaster WHERE Short_Name = 'HSO 1' AND [Sales Offcie Type] = 'Sales Office'), TargetVolume AS (SELECT SUM(BudgetQuantityLPD) AS TargetVolume FROM Dw.fSalesBudget b INNER JOIN PlantID p ON b.SalesOfficeID = p.PLANT WHERE PlanMonth = '202504'), TotalVolume AS (SELECT SUM(SalesQuantity) / 30.0 AS TotalVolume FROM Dw.fSales f INNER JOIN PlantID p ON f.SalesOfficeID = p.PLANT WHERE BillingDate BETWEEN '2025-04-01' AND '2025-04-30') SELECT t.TargetVolume, s.TotalVolume, CASE WHEN t.TargetVolume = 0 THEN NULL ELSE (s.TotalVolume / t.TargetVolume) * 100 END AS AchievementPercent FROM TargetVolume t, TotalVolume s;
            3) what is the budget of TG 2 in apil 2025 ? -> sql -> WITH PlantID AS (SELECT PLANT FROM Dw.dSalesOfficeMaster WHERE REGION_NAME = 'TG-2' AND [Sales Offcie Type] = 'Sales Office'), TargetVolume AS (SELECT SUM(BudgetQuantityLPD) AS TargetVolume FROM Dw.fSalesBudget b INNER JOIN PlantID p ON b.SalesOfficeID = p.PLANT WHERE PlanMonth = '202504'), TotalVolume AS (SELECT SUM(SalesQuantity) / 30.0 AS TotalVolume FROM Dw.fSales f INNER JOIN PlantID p ON f.SalesOfficeID = p.PLANT WHERE BillingDate BETWEEN '2025-04-01' AND '2025-04-30') SELECT t.TargetVolume, s.TotalVolume, CASE WHEN t.TargetVolume = 0 THEN NULL ELSE (s.TotalVolume / t.TargetVolume) * 100 END AS AchievementPercent FROM TargetVolume t, TotalVolume s; 
            *4) what is the budget of april 2025? sql -> WITH PlantID AS (SELECT PLANT FROM Dw.dSalesOfficeMaster WHERE [Sales Offcie Type] = 'Sales Office'), TargetVolume AS (SELECT SUM(BudgetQuantityLPD) AS TargetVolume FROM Dw.fSalesBudget b INNER JOIN PlantID p ON b.SalesOfficeID = p.PLANT WHERE PlanMonth = '202504'), TotalVolume AS (SELECT SUM(SalesQuantity) / 30.0 AS TotalVolume FROM Dw.fSales f INNER JOIN PlantID p ON f.SalesOfficeID = p.PLANT WHERE BillingDate BETWEEN '2025-04-01' AND '2025-04-30') SELECT t.TargetVolume, s.TotalVolume, CASE WHEN t.TargetVolume = 0 THEN NULL ELSE (s.TotalVolume / t.TargetVolume) * 100 END AS AchievementPercent FROM TargetVolume t, TotalVolume s;
    **IMP 5) what is the budget of lastweek -> sql -> WITH PlantID AS (SELECT PLANT FROM Dw.dSalesOfficeMaster WHERE [Sales Offcie Type] = 'Sales Office'), TargetVolume AS (SELECT SUM(BudgetQuantityLPD) AS TargetVolume FROM Dw.fSalesBudget b INNER JOIN PlantID p ON b.SalesOfficeID = p.PLANT WHERE PlanMonth = FORMAT(DATEADD(DAY, -1, DATEADD(WEEK, DATEDIFF(WEEK, 0, GETDATE()), 0)), 'yyyyMM')), TotalVolume AS (SELECT SUM(SalesQuantity) / 7.0 AS TotalVolume FROM Dw.fSales f INNER JOIN PlantID p ON f.SalesOfficeID = p.PLANT WHERE BillingDate BETWEEN DATEADD(DAY, -7, DATEADD(WEEK, DATEDIFF(WEEK, 0, GETDATE()), 0)) AND DATEADD(DAY, -1, DATEADD(WEEK, DATEDIFF(WEEK, 0, GETDATE()), 0))) SELECT t.TargetVolume, s.TotalVolume, CASE WHEN t.TargetVolume = 0 THEN NULL ELSE (s.TotalVolume / t.TargetVolume) * 100 END AS AchievementPercent FROM TargetVolume t, TotalVolume s;
            6)what is the budget of may 2025 vs april 2025 ? SQL: WITH PlantID AS (SELECT PLANT FROM Dw.dSalesOfficeMaster WHERE [Sales Offcie Type] = 'Sales Office'), AprilBudget AS (SELECT SUM(BudgetQuantityLPD) AS TargetVolume FROM Dw.fSalesBudget b INNER JOIN PlantID p ON b.SalesOfficeID = p.PLANT WHERE PlanMonth = '202504'), AprilActual AS (SELECT SUM(SalesQuantity) / 30.0 AS TotalVolume FROM Dw.fSales f INNER JOIN PlantID p ON f.SalesOfficeID = p.PLANT WHERE BillingDate BETWEEN '2025-04-01' AND '2025-04-30'), MayBudget AS (SELECT SUM(BudgetQuantityLPD) AS TargetVolume FROM Dw.fSalesBudget b INNER JOIN PlantID p ON b.SalesOfficeID = p.PLANT WHERE PlanMonth = '202505'), MayActual AS (SELECT SUM(SalesQuantity) / 31.0 AS TotalVolume FROM Dw.fSales f INNER JOIN PlantID p ON f.SalesOfficeID = p.PLANT WHERE BillingDate BETWEEN '2025-05-01' AND '2025-05-31') SELECT a.TargetVolume AS AprilBudget, aa.TotalVolume AS AprilActual, CASE WHEN a.TargetVolume = 0 THEN NULL ELSE (aa.TotalVolume / a.TargetVolume) * 100 END AS AprilAchievementPercent, m.TargetVolume AS MayBudget, ma.TotalVolume AS MayActual, CASE WHEN m.TargetVolume = 0 THEN NULL ELSE (ma.TotalVolume / m.TargetVolume) * 100 END AS MayAchievementPercent, CASE WHEN a.TargetVolume = 0 OR m.TargetVolume = 0 THEN NULL ELSE ((ma.TotalVolume / m.TargetVolume) - (aa.TotalVolume / a.TargetVolume)) * 100 END AS AchievementGrowthPercent FROM AprilBudget a, AprilActual aa, MayBudget m, MayActual ma;

For every budget-related query, always calculate target, actual, and achievement; apply filters based on user input—use d.CITY if city mentioned, d.REGION_NAME if region mentioned, f.ProductHeirachy1 if product mentioned (otherwise no product filter); convert user month to PlanMonth in YYYYMM format; compute actual as total sales quantity divided by exact days in that month; and if a product is mentioned, join with DW.fSalesBudget on material code to calculate target, actual, and achievement accordingly.
...
- Output queries that are complete, correct, and production-ready.
    ...
    **Important Note: The following examples cover common ways users may ask about UBC  If a user query differs in wording or format, interpret the intent and generate the SQL by adapting the logic shown in these examples. Always use them as reference patterns for solving similar queries.**
    ...
    MODEL 1: UNIQUE BILLING COUNT OR UNIQUE BILLING CUSTOMERS: -> ubc or unique billing count or unique billing customers:
        ...
        3) what is unique billed customers yesterday ? -> SQl: SELECT COUNT(DISTINCT BillingDocument) AS UniqueBillingCount_Yesterday FROM Dw.fsales WHERE CAST(BillingDate AS DATE) = CAST(GETDATE() - 1 AS DATE);
...
For every budget-related query, always calculate target, actual, and achievement; apply filters based on user input—use d.CITY if city mentioned, d.REGION_NAME if region mentioned, f.ProductHeirachy1 if product mentioned (otherwise no product filter); convert user month to PlanMonth in YYYYMM format; compute actual as total sales quantity divided by exact days in that month; and if a product is mentioned, join with DW.fSalesBudget on material code to calculate target, actual, and achievement accordingly.
    ...
    =>USECASE 2: **opm(over previous month) and opy(over previous year)
    Examples:
    1)what is the budget of may 2025 vs april 2025 ? SQL: WITH PlantID AS (SELECT PLANT FROM Dw.dSalesOfficeMaster WHERE [Sales Offcie Type] = 'Sales Office'), AprilBudget AS (SELECT SUM(BudgetQuantityLPD) AS TargetVolume FROM Dw.fSalesBudget b INNER JOIN PlantID p ON b.SalesOfficeID = p.PLANT WHERE PlanMonth = '202504'), AprilActual AS (SELECT SUM(SalesQuantity) / 30.0 AS TotalVolume FROM Dw.fSales f INNER JOIN PlantID p ON f.SalesOfficeID = p.PLANT WHERE BillingDate BETWEEN '2025-04-01' AND '2025-04-30'), MayBudget AS (SELECT SUM(BudgetQuantityLPD) AS TargetVolume FROM Dw.fSalesBudget b INNER JOIN PlantID p ON b.SalesOfficeID = p.PLANT WHERE PlanMonth = '202505'), MayActual AS (SELECT SUM(SalesQuantity) / 31.0 AS TotalVolume FROM Dw.fSales f INNER JOIN PlantID p ON f.SalesOfficeID = p.PLANT WHERE BillingDate BETWEEN '2025-05-01' AND '2025-05-31') SELECT a.TargetVolume AS AprilBudget, aa.TotalVolume AS AprilActual, CASE WHEN a.TargetVolume = 0 THEN NULL ELSE (aa.TotalVolume / a.TargetVolume) * 100 END AS AprilAchievementPercent, m.TargetVolume AS MayBudget, ma.TotalVolume AS MayActual, CASE WHEN m.TargetVolume = 0 THEN NULL ELSE (ma.TotalVolume / m.TargetVolume) * 100 END AS MayAchievementPercent, CASE WHEN a.TargetVolume = 0 OR m.TargetVolume = 0 THEN NULL ELSE ((ma.TotalVolume / m.TargetVolume) - (aa.TotalVolume / a.TargetVolume)) * 100 END AS AchievementGrowthPercent FROM AprilBudget a, AprilActual aa, MayBudget m, MayActual ma;
    2)SQL: WITH MilkMaterialCodes AS (SELECT DISTINCT MaterialCode FROM DW.fSales WHERE ProductHeirachy1 = 'Milk' AND ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others')), AprilActual AS (SELECT SUM(f.SalesQuantity)/30.0 AS actual FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = f.SalesOfficeID INNER JOIN MilkMaterialCodes m ON f.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND f.BillingDate BETWEEN '2025-04-01' AND '2025-04-30'), AprilTarget AS (SELECT SUM(b.BudgetQuantityLPD) AS target FROM DW.fSalesBudget b INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = b.SalesOfficeID INNER JOIN MilkMaterialCodes m ON b.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND b.PlanMonth = '202504'), MayActual AS (SELECT SUM(f.SalesQuantity)/31.0 AS actual FROM DW.fSales f INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = f.SalesOfficeID INNER JOIN MilkMaterialCodes m ON f.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND f.BillingDate BETWEEN '2025-05-01' AND '2025-05-31'), MayTarget AS (SELECT SUM(b.BudgetQuantityLPD) AS target FROM DW.fSalesBudget b INNER JOIN DW.dSalesOfficeMaster d ON d.PLANT = b.SalesOfficeID INNER JOIN MilkMaterialCodes m ON b.MaterialCode = m.MaterialCode WHERE d.[Sales Offcie Type] = 'sales Office' AND b.PlanMonth = '202505') SELECT a.actual AS AprilActual, t.target AS AprilTarget, CASE WHEN t.target = 0 THEN NULL ELSE (a.actual / t.target) * 100 END AS AprilAchievementPercent, ma.actual AS MayActual, mt.target AS MayTarget, CASE WHEN mt.target = 0 THEN NULL ELSE (ma.actual / mt.target) * 100 END AS MayAchievementPercent, CASE WHEN t.target = 0 OR mt.target = 0 THEN NULL ELSE ((ma.actual / mt.target) - (a.actual / t.target)) * 100 END AS AchievementGrowthPercent FROM AprilActual a, AprilTarget t, MayActual ma, MayTarget mt;
...
def detect_intent(user_query: str) -> str:
    ...
    budget_terms=["budget", "target", "achievement", "actual","goal","actual and target"]
    ...
    for val, col in matched_entities.items():
        preprocessed_query += (
            f" '{val}' is mapped to the column '{col}'. "
             f" Use this column to filter results as per the user's original query.\n"
